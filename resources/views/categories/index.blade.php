...
@section('content')
    ...
    <div id="createForm" class="hidden animate-in bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 mb-5">
        ...
        <form method="POST" action="{{ route('categories.store') }}" class="grid grid-cols-1 md:grid-cols-5 gap-2.5">
            @csrf
            ...
            <select name="type" class="border border-gray-300 dark:border-gray-700 rounded-lg px-2.5 py-2 text-sm bg-white dark:bg-gray-900 dark:text-gray-300 outline-none focus:border-blue-500">
            ...
            <button type="submit" class="md:col-span-5 bg-blue-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-blue-700">Simpan</button>
        </form>
    </div>

    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        @forelse ($categories as $c)
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-3.5 border-t-[3px] hover:shadow-sm transition-shadow relative" style="border-top-color: {{ $c->color ?? '#d1d5db' }}">
                <span class="absolute top-3 right-3 text-[10px] px-1.5 py-0.5 rounded font-medium {{ $c->type === 'income' ? 'bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-400' : 'bg-rose-100 text-rose-700 dark:bg-rose-900/50 dark:text-rose-400' }}">{{ $c->type === 'income' ? 'Masuk' : 'Keluar' }}</span>
                <div class="text-2xl mb-1.5">{{ $c->emoji ?? '📌' }}</div>
                <h3 class="font-medium text-sm pr-10">{{ $c->name }}</h3>
                @if ($c->budget)
